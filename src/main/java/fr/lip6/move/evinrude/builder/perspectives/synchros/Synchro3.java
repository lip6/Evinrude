 ...
 */
public class Synchro3 extends AbstractRule implements IRule, IRuleInstruction {
	public static final String NAME = "Synchro2: Mutex Init";
	...
	public final List<ISubModel> process(ResultDep dependencies, IInstruction instruction) {
		...
		if (!call.getFunctionName().equalsIgnoreCase("pthread_mutex_init")) { return results; }
		...
		for (ISubModel mutexSubModel : dependencies.getAssociatedSubModels(instruction)) {
			...
			// La transition MUTEX_INIT
			ITransition init = mutexSubModel.getTransition("sys_" + id(call));
			...
			// L'arc qui permet de positionner la valeur du mutex
			IArc fetch = mutexSubModel.createArc(init, semaphoresResource);
			fetch.setValuation(new Valuation(IValuation.EVENT_RESOURCE_IDENTIFIER, "1", IValuation.NO_OPERATOR));
