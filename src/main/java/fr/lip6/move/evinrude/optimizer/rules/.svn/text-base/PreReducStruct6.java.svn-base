...
/**
 * Définition d'une réduction structurelle hiérarchique : <b>Dummy Submodels</b><br>
 * Cette réduction est appliquée sur un sous-modèle lorsque celui-ci ne contient qu'une transition structurelle.
 *
 * @author Jean-Baptiste Voron
 */
public class PreReducStruct6 extends Reduction {

	/**
	 * Constructeur
	 */
	public PreReducStruct6() {
		super("Dummy Submodels");
		...
		addCondition(new TypedPlaceCondition(ICondition.PRED, IPlace.INPUT));
		addCondition(new SingleSuccessor(ICondition.PRED));
		addCondition(new NoMarkingValue(ICondition.PRED));

		addCondition(new TypedPlaceCondition(ICondition.SUCC, IPlace.OUTPUT));
	...
	public final void performReduction(IModel model, INode self) throws EvinrudeException {
		...
		LOG.fine("\tSuppression du submodel" + self.getSubModelContainer().getInstructionNumber());
		self.getSubModelContainer().getParentPlace().removeSubModel(self.getSubModelContainer().getInstructionNumber());
	}
}
